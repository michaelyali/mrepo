...
class MainCommands {
    static load(program, configFile) {
        ...
        program
            ...
            .action(async (packageName, options) => runBuild(packageName, options.cache));
        program
        ...
        async function runBuild(packageName, useCache = true) {
            const exec = async (pName) => {
                const hasCache = await helpers_1.hasCached(workspaceName, packageName);
                if (!useCache || !hasCache) {
                    ...
                    await helpers_1.saveCache(workspaceName, packageName);
                }
                else {
                    utils_1.logger.info(AvailableCommands.BUILD, `Package ${color.italic.bold(pName)} has been built already`, emoji.get(':ok_hand:'));
                }
            };
            if (!packageName) {
