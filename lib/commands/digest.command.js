...
class DigestCommand {
                    ...
                    if (util_1.isArrayFull(digestPackages)) {
                        ...
                        switch (mode) {
                            ...
                            case constants_1.DIGEST_MODE.LINK:
                                ...
                                break;
                            case constants_1.DIGEST_MODE.COPY:
    ...
    static removeFromNodeModules(mrepoConfig, targetPath, digestPackages, lib = false) {
        const packagesStr = digestPackages
            .map((p) => path_1.join(targetPath.path, 'node_modules', `@${mrepoConfig.workspace.scope}`, p, lib ? 'lib' : ''))
            .join(' ');
        child_process_1.execSync(`npx rimraf ${packagesStr}`, {
            stdio: 'ignore',
        });
    }
    static runInstall(mrepoConfig, mrepoPath, targetPath, target, digestPackages, options) {
        ...
        const packagesStr = digestPackages
            ...
            const version = options.withVersion
                ...
                : options.withLocalVersions
                    ? utils_1.loadPackageJsonVersion(path_1.join(mrepoPath.path, mrepoConfig.workspace.name, p, 'package.json'))
                    : '';
    ...
    static async runCopy(mrepoConfig, mrepoPath, targetPath, digestPackages, options) {
        this.removeFromNodeModules(mrepoConfig, targetPath, digestPackages, true);
        for (const digestPackage of digestPackages) {
            const sourcePath = path_1.join(mrepoPath.path, mrepoConfig.workspace.name, digestPackage, 'lib', '**', '*');
            const destPath = path_1.join(targetPath.path, 'node_modules', `@${mrepoConfig.workspace.scope}`, digestPackage, 'lib');
            child_process_1.execSync(`npx cpx "${sourcePath}" "${destPath}"`, {
                stdio: options.quiet ? 'ignore' : 'inherit',
                shell: '/bin/bash',
                env: process.env,
            });
